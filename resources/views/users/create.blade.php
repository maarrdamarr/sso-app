...
@section('content')
  ...
  <form action="{{ route('users.store') }}" method="post" class="bg-white p-3 rounded">
    @csrf
    <div class="row g-3">
        <div class="col-md-6">
        ...
        {{-- role: kalau yang login HR, kita tidak tampilkan pilihan --}}
        @unless(auth()->user()->hasRole('hr'))
            <div class="col-md-12">
                <label class="form-label d-block mb-1">Role</label>
                @foreach ($roles as $r)
                    <div class="form-check form-check-inline">
                        <input type="checkbox" name="roles[]" value="{{ $r->id }}" class="form-check-input" id="r{{ $r->id }}">
